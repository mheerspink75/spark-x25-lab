...
| --- | --- | --- | --- |
| `numpy` | `>=1.24.0` | `test_prompts/memory_stress_test/benchmark.py`, `github_issue_triage/src/issue_triage/charts.py` | Numerical analysis and chart data processing in benchmark and chart modules. |
| `matplotlib` | `>=3.7.0` | `test_prompts/memory_stress_test/benchmark.py`, `github_issue_triage/src/issue_triage/charts.py` | Chart generation (PNG output) in both the benchmark and the triage chart modules. |
| `flask` | `>=3.0.0` | `local_web_dashboard/dashboard.py` | Web application framework serving the dashboard and REST API endpoints. |
| `psutil` | `>=6.0.0` | `local_web_dashboard/dashboard.py` | System resource monitoring (CPU, RAM, disk) for the dashboard. |

**Requirement files:**
- `test_prompts/memory_stress_test/requirements.txt` — lists `matplotlib>=3.7.0`, `numpy>=1.24.0`.
- `local_web_dashboard/requirements.txt` — lists `flask>=3.0.0`, `psutil>=6.0.0`.
- `github_issue_triage/requirements.txt` — lists `matplotlib>=3.7.0`.
...
### `matplotlib>=3.7.0`
**Role:** Chart generation and data visualization. Used in `benchmark.py` to generate performance charts (PNG) from benchmark metrics, and in `github_issue_triage/src/issue_triage/charts.py` to create category distribution and count charts for the triage report.
...
### `flask>=3.0.0`
**Role:** Web application framework. Serves the `local_web_dashboard/dashboard.py` Flask application, providing REST API endpoints (`/`, `/api/metrics`, `/health`) for system resource monitoring and health checks.
