...
- **Benchmark code:** `test_prompts/prompt1_memory_stress_test/benchmark.py` implements a CPU performance test using a timed loop under fixed workload, configured with `CPU_CORES = 4`.
...
- **Benchmark code:** `test_prompts/prompt1_memory_stress_test/benchmark.py` implements a memory stress test with a 64 MB shared buffer accessed repeatedly by multiple threads.
...
- **Reporting file:** `test_prompts/prompt1_memory_stress_test/REPORT.md` documents the memory performance results in section 2.2.
...
- **Benchmark code:** `test_prompts/prompt1_memory_stress_test/benchmark.py` implements a file I/O performance test writing and reading a 20-file I/O load sequence across repeated iterations.
...
- **Model health check:** `run.sh` and `prompt4_local_web_dashboard/dashboard.py` use `curl` to query `/api/tags` and `/api/generate` for health verification.
- **Dashboard API:** `prompt4_local_web_dashboard/dashboard.py` exposes REST endpoints (`/`, `/api/metrics`, `/health`) using `jsonify` for JSON responses.
...
- **Multithreaded benchmark:** `test_prompts/prompt1_memory_stress_test/benchmark.py` uses `ThreadPoolExecutor` with `THREAD_COUNT = 4` for multithreaded CPU and memory workloads.
